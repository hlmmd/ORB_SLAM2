...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp)
{
    ...
    if ((fabs(1584792968.012194 - mCurrentFrame.mTimeStamp) < 1e-3)) //video2
    {
        if (mState != OK)
        {
            cout << "initial error , try restart" << endl;
            exit(0);
        }

        ofstream fout;
        fout.open("MapPoint.txt");

        // cout << fixed << setprecision(6) << mCurrentFrame.mTimeStamp << endl;
...
void Tracking::cvtcolor(cv::Mat &ImGray)
{
...
void Tracking::CompareTwoFrame(const cv::Mat &im1, const double &timestamp1,
                               const cv::Mat &im2, const double &timestamp2)
{
    cv::Mat ImGray1 = im1;
    cv::Mat ImGray2 = im2;
    ...
    Frame Frame1 = Frame(ImGray1, timestamp1, mpIniORBextractor, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
    Frame Frame2 = Frame(ImGray1, timestamp1, mpIniORBextractor, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth);
...
void Tracking::Track()
{
    ...
    {
        // hlmmd change
        // fprintf(stdout, "%ld %d %d\n", mCurrentFrame.mvKeys.size(), mCurrentFrame.mDescriptors.rows, mCurrentFrame.mDescriptors.cols);

        // cout << mCurrentFrame.mvKeys[0].pt.x << " " << mCurrentFrame.mvKeys[0].pt.y << endl;
        // for (int i = 0; i < mCurrentFrame.mDescriptors.cols; i++)
        //     cout << mCurrentFrame.mDescriptors.at<float>(0, i) << " ";
        // cout << endl;
...
void Tracking::MonocularInitialization()
{
    ...
    // {
    //     for (int j = 0; j < mCurrentFrame.mDescriptors.cols; j++)
    //         cout << mCurrentFrame.mDescriptors.at<float>(i, j) << ' ';
    //     cout << endl;
    // }
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // cout << pKFini->mnFrameId << endl;
    // cout << pKFcur->mnFrameId << endl;
